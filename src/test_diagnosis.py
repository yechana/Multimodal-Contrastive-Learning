...
'''
Notes for Adhham:
/scratch/em4449/stanford_data/CheXpert-v1.0/ = Original CheXpert data and splits
/scratch/yl7971/capstone/CheXpert-v1.0/ = pre-processed (but not augmented) data, and multiple versions of split csvs
    - seth_train/val/test = How Manning paper did it
    - seth_train/val/test_bal = multilabel balanced splits by order of 2 (can largely ignore)
    - seth_train/val/test_bal5 = multilabel balanced splits by order of 5 (since we have 5 classes)
    - seth_train/val/test_bal5_ds = hacky downsampled version of bal5, I tried just getting rid of all rows with False in all 5 cols
                                    The highest pos_weight decreased from ~14 to ~8.3, lost about 87000 rows though.
    - seth_train/val/test_bal5_ds2 = manually downsampled version of bal5, largest pos_weight = 8, and lost about 90k rows.
'/scratch/yl7971/capstone/CheXpert-Aug/' = pre-processed and augmented images. I currently pull my training images from here.
    - To look at the pre-processing and augmentation steps, look at DT_Diagnosis_preprocess.py and DT_Diagnosis_preprocess_aug.py
    
To Run: sbatch run_DT.s

When it finishes running, it produces 3 lightning logs per run:
1) The "warming up clasification head" part, for just 200 steps
2) Tuning all weights (what I mainly look at)
3) Evaulation on the test set (I also have it separately write out just the test losses and AUC to a txt file for ease of access)
'''
# UPDATED TO USE ORDER 5 BLANACED CSVS
# UPDATED TO REPORT TRAIN AND VAL AUROC
# UPDATED TO USE POS_WEIGHT IN LOSS FUCTION
# UPDATED TO USE AUGMENTED IMAGES FOR TRAINING
# UPDATED TO ACCOMODATE USING OUR PRETRAINED WEIGHTS
# Use lower LR (original: 1e-4 and 1e-6) -- REVERTED
# Increased dropout to 0.5 from 0.2 -- REVERTED
# Use ds splits (no all false rows) -- REVERTED
# Use manual ds (ver 2) splits
...
drd = '/scratch/arz8448/capstone/outputs/diagnosis_test/'
...
#print('SETUP DATA')
ckpt = '../outputs/diagnosis/lightning_logs/version_5/checkpoints/epoch=20-step=71672.ckpt'
...
test_res = trainer.test(model=model, datamodule=data, ckpt_path=ckpt)
